...
int cmd_mv (int argcnt, char *argvec[])
	...
	if(argcnt < 3)
		{
		printf("mv: No destination location provided");
		return (-1);
		}
	if(argcnt > 3)
		{
	...
	char *cp_argvec[] = {"cp", argvec[1], argvec[2]};
	int cp_return = cmd_cp(3, cp_argvec);
	if(cp_return < 0)
		{
		printf("error in cp within mv");
		return (-1);
		}
	char *rm_argvec[] = {"rm", argvec[1]};
	int rm_return = cmd_rm(2, rm_argvec);
	return rm_return;
#endif
	return 0;
	}
